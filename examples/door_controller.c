...
door_state_t get_sensor_state(door_sensor_t *sensor){
    if (sensor == NULL){
        return DOOR_INVALID;
    } else{
        return sensor->state;
...
fsm_status_t lock_the_door(door_sensor_t* door_sensor){
...
fsm_status_t unlock_the_door(door_sensor_t* door_sensor){
...
/* Simulate giving command to close the door */
fsm_status_t close_the_door(door_sensor_t* door_sensor){
    /* Simulate giving command to close the door */
    if (door_sensor == NULL){
        return FSM_ERROR_INVALID_ARG;
    } else {
